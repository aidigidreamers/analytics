...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://swap.thexdao.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83' ? 'FTM' : token0Address}/${'FTM'}`
    )
  } else {
    return (
      `https://swap.thexdao.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83' ? 'FTM' : token0Address}/${
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.thexdao.com/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.thexdao.com/#/swap?inputCurrency=${
...
export function getMiningPoolLink(token0Address) {
  return `https://swap.thexdao.com/#/uni/FTM/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://swap.thexdao.com/#/uni'
  if (!linkVariable) {
